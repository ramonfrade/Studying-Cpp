...
namespace PPP
{
    ...
    simple_error(std::string s) // write ``error: s'' and exit program (for non-exception terminating error handling)
    {
        std::cerr << "error: " << s << '\n';
        exit(1);
    }
    ...
    PPP_EXPORT enum class Error_action
    {
        ignore,
        throwing,
        terminating,
    ...
    PPP_EXPORT void seed(int s)
    {
        get_rand().seed(s);
    }

    PPP_EXPORT void seed()
    {
        get_rand().seed();
    }
