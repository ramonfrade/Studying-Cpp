...
#include "ppp3/graphics/Image_private.h"
...
namespace Graph_lib
{
    ...
    void Open_polyline::draw_specifics(Painter& painter) const
    {
        if(color().visibility() && 1 < number_of_points()) // draw sole pixel?
            for(int i = 1; i < number_of_points(); ++i) painter.draw_line(point(i - 1), point(i));
    }
    ...
    void Lines::draw_specifics(Painter& painter) const
    {
        ...
        if(color().visibility())
            for(int i = 1; i < number_of_points(); i += 2) painter.draw_line(point(i - 1), point(i));
    }
        ...
        : label(Point{0, 0}, lab), line(xy, d == x ? Point{xy.x + length, xy.y} : Point{xy.x, xy.y - length})
    {
        ...
        switch(d)
        {
            ...
            case Axis::z: error("z axis not implemented");
        }
    }
    ...
    void Marked_polyline::draw_specifics(Painter& painter) const
    {
        ...
        painter.set_font(font());
        painter.set_font_size(font_size());
        for(int i = 0; i < number_of_points(); ++i) draw_mark(painter, point(i), mark[i % mark.size()]);
    }
