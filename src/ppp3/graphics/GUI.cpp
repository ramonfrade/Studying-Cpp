#include "ppp3/graphics/GUI.h"
#include "ppp3/graphics/GUI_private.h"
#include "ppp3/graphics/Window.h"
#include "ppp3/internal/std_lib_facilities.h"
#include <sstream>
...
Button::Button(Point xy, int w, int h, const string& label, Callback cb) : Widget(xy, w, h, label, cb)
{
...
In_box::In_box(Point xy, int w, int h, const string& s, Callback cb) : Widget(xy, w, h, s, 0)
{
    WidgetPrivate& w_impl = get_impl();
...
void wait_for_input(InputDialog* dialog, In_box::State& state)
{
    ...
    auto conn1 = QObject::connect(dialog,
                                  &QInputDialog::accepted,
                                  [&]
                                  {
    ...
    auto conn2 = QObject::connect(dialog,
                                  &QInputDialog::rejected,
                                  [&]
                                  {
...
Menu::Menu(Point xy, int w, int h, Kind kk, const string& label) : Widget(xy, w, h, label, 0), k(kk), offset(0)
{
    WidgetPrivate& w_impl = get_impl();
