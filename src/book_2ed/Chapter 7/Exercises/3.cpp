...
vector<Variable> names{Variable("k", 1000, true), Variable("pi", 3.14, true)};
...
double set_value(string s, double d) // function that set a new value to a variable
{
    for(int i = 0; i <= names.size(); ++i)
        if(names[i].name == s)
        {
            if(names[i].is_constant == true)
            {
...
double const_declaration() // declares a variable
{
    Token t = ts.get();
    if(t.kind != 'a')
...
double statement() // declares or calculate a variable
{
    ...
    switch(t.kind)
    {
        case constant: return const_declaration();
